...
export async function runJarvisAgentSmokeV1({
  ...
  const resultPath = path.join(dir, "room-seat-smoke-v8.json");
  if (fs.existsSync(resultPath)) {
    console.log("[agent-smoke-v8] prior result exists; skipping");
  ...
  for (const [agentId, marker] of seats) {
    ...
    results.push(result);
    console.log("[agent-smoke-v8] seat " + JSON.stringify(result));
  }
  ...
  console.log("[agent-smoke-v8] completed " + JSON.stringify({
    pass: summary.pass,
