...
export function researchPaths() {
  ...
  return {
    ...
    semanticVerifications:path.join(r,"ledger","semantic-verifications"),
...
function initDirs(){ const p=researchPaths(); [p.root,p.briefs,p.packets,p.merges,p.verifications,p.semanticVerifications,path.dirname(p.events),p.dossiers,p.docs,p.sources,p.inbox,p.runs,p.diagnostics].forEach(ensureDir); return p; }
...
export function buildResearchDossier(briefId) {
  ...
  let verification=null,semanticVerification=null;
  try{verification=JSON.parse(fs.readFileSync(path.join(p.verifications,briefId+".json"),"utf8"));}catch{}
  try{semanticVerification=JSON.parse(fs.readFileSync(path.join(p.semanticVerifications,briefId+".json"),"utf8"));}catch{}
  ...
  const d={
    ...
    semantic_support_summary:semanticVerification?{semantic_verification_id:semanticVerification.semantic_verification_id,...(semanticVerification.summary||{})}:null,
...
export function getResearchStatus(){
  ...
  return{
    ...
    semantic_verifications:fs.readdirSync(p.semanticVerifications).filter(x=>x.endsWith(".json")).length,
